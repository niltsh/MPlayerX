...
fromHardwareWithAttributes:(NSMutableDictionary *)attributes
{
	...
	if (isPressed) {
		...
		if (target && action) {
			...
			[self simulateEvent:[NSArray arrayWithObjects:target, action, event, nil]];
		}		
	} else {
		// 如果是放开键
		repeatCanceled = YES;
...
-(void) simulateEvent:(NSArray *)arg
{
	...
	SEL sel = (SEL)[arg objectAtIndex:1];
	...
	if (repeatEntered) {		
		...
		if (repeatCounter != kSCMRepeatCounterThreshold) {
			...
			[self performSelector:@selector(simulateEvent:)
					   withObject:[NSArray arrayWithObjects:tgt, sel, newEv, nil]
