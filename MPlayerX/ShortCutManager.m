...
fromHardwareWithAttributes:(NSMutableDictionary *)attributes
{
	...
	if (isPressed) {
		...
		if (target && action) {
			...
			[self simulateEvent:[NSArray arrayWithObjects:target, [NSValue valueWithPointer:action], event, nil]];
		}		
	} else {
		// 如果是放开键
		repeatCanceled = YES;
...
-(void) simulateEvent:(NSArray *)arg
{
	...
	SEL sel = [[arg objectAtIndex:1] pointerValue];
	...
	if (repeatEntered) {		
		...
		if (repeatCounter != kSCMRepeatCounterThreshold) {
			...
			[self performSelector:@selector(simulateEvent:)
					   withObject:[NSArray arrayWithObjects:tgt, [NSValue valueWithPointer:sel], newEv, nil]
