...
fromHardwareWithAttributes:(NSMutableDictionary *)attributes
{
	...
	if (isPressed) {
		...
		if (target && action) {
			...
			[self simulateEvent:[NSArray arrayWithObjects:target, [NSString stringWithUTF8String:sel_getName(action)], event, nil]];
		}		
	} else {
		// 如果是放开键
		repeatCanceled = YES;
...
-(void) simulateEvent:(NSArray *)arg
{
	...
	SEL sel = sel_getUid([[arg objectAtIndex:1] UTF8String]);
	...
	if (repeatEntered) {		
		...
		if (repeatCounter != kSCMRepeatCounterThreshold) {
			...
			[self performSelector:@selector(simulateEvent:)
					   withObject:[NSArray arrayWithObjects:tgt, [NSString stringWithUTF8String:sel_getName(sel)], newEv, nil]
